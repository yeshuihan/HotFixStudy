...
object AESUtils {
     ...
     */
    fun encryptDexFile(file: File, outFilePath: String, password: String = DEFAULT_PASSWORD) {
        ...
        try {
            ...
            writeFile.write(encrypt(buff, password))
        } catch (e: Exception) {
            e.printStackTrace()
        } finally {
            readFile?.let {
    ...
    fun decryptDexFile(file: File, outFilePath: String, password: String = DEFAULT_PASSWORD) {
        val outFile = File(outFilePath)
        outFile.parentFile.mkdirs()
        ...
        try {
            ...
            val afterArray = decrypt(byteArray, password)
